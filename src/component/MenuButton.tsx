...
export default function MenuButton({ children, className, variant = "inactive" }: MenuButtonProps) {
    ...
    return (
        ...
        //     {children}
        // </button>
        <button ref={ref} className={clsx("text-white rounded-xl py-2 m-0", variant == "inactive" ? "text-sm" : "text-xl", className)} onMouseEnter={onMouseEnterHandler} onMouseLeave={onMouseLeaveHandler}>
            {children}
        </button>
    )
}